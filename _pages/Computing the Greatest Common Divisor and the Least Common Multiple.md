...
```c
//这里分解质因子时做了一个时间复杂度上的优化
//由于正整数最多只会含有一个大于sqrt(x)的质因子(若有两个,则相乘一定大于x,矛盾)
//所以我们在循环内只处理小于等于sqrt(x)的质因子, 剩余大于sqrt(x)的因子只能被大于sqrt(x)的质因子所分解,
//又由于大于sqrt(x)的质因子最多一个, 即该因子就是大于sqrt(x)的质因子, 个数为1
void divide(int x)
{
    for (int i = 2; i <= sqrt(x); i ++ ) //先处理小于sqrt(x)的的质因子
    {
        if (x % i == 0)
        {
            int cnt = 0;
