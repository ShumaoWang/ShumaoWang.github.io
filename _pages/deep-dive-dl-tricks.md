---
permalink: /deep-dive/dl-tricks/
title: "Deep Learning Models and Training Tricks"
excerpt: ""
author_profile: false
---

# Deep Learning Models and Training Tricks

<div class="toc-mobile">
  <input class="toc-toggle" type="checkbox" id="toc-toggle">
  <label class="toc-toggle__button" for="toc-toggle"><i class="fas fa-list" aria-hidden="true"></i><span class="sr-only">Contents</span></label>
  <label class="toc-backdrop" for="toc-toggle"></label>
  <div class="toc-drawer" role="dialog" aria-labelledby="toc-mobile-title">
    <div class="toc-drawer__inner">
      <p id="toc-mobile-title" class="toc-block__title">Table of Contents</p>
      <ul class="toc-block__list">
        <li><a href="#overview">Overview</a></li>
        <li><a href="#model-zoo">Model Zoo</a></li>
        <li><a href="#training-tricks">Training Tricks</a></li>
        <li><a href="#debugging-checklist">Debugging Checklist</a></li>
        <li><a href="#key-takeaways">Key Takeaways</a></li>
      </ul>
    </div>
  </div>
</div>

<div class="article-layout">
  <div class="article-main" markdown="1">
This page collects model summaries, training recipes, and practical lessons from experiments.

## Overview {#overview}
What I am learning and why it matters.

## Model Zoo {#model-zoo}
Summaries of common model families and when to use them.

## Training Tricks {#training-tricks}
Warmup, schedules, regularization, and stability tips.

## Debugging Checklist {#debugging-checklist}
Things I check when training is unstable.

## Key Takeaways {#key-takeaways}
- Coming soon.

</div>
  <aside class="article-toc">
    <details class="toc-panel" open>
      <summary class="toc-panel__toggle">Contents</summary>
      <div class="toc-block">
        <p class="toc-block__title">Table of Contents</p>
        <ul class="toc-block__list">
          <li><a href="#overview">Overview</a></li>
          <li><a href="#model-zoo">Model Zoo</a></li>
          <li><a href="#training-tricks">Training Tricks</a></li>
          <li><a href="#debugging-checklist">Debugging Checklist</a></li>
          <li><a href="#key-takeaways">Key Takeaways</a></li>
        </ul>
      </div>
    </details>
  </aside>
</div>
